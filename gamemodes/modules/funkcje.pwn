...
sendTipDialogMessage(id, string:msg[]) {
	...
	return ShowPlayerDialogEx(id, DIALOG_ID_NO_RESPONSE, DIALOG_STYLE_MSGBOX, "{8FCB04}Lisek-RP{FFFFFF} ª Informacja", _str, "Zamknij", "");
	//return SendClientMessage(id, color, _str);
}

sendErrorDialogMessage(id, string:msg[]) {
	format(_str,128,"%s", msg);
	return ShowPlayerDialogEx(id, DIALOG_ID_NO_RESPONSE, DIALOG_STYLE_MSGBOX, "{8FCB04}Lisek-RP{FFFFFF} ª Wystπpi≥ b≥πd", _str, "Zamknij", "");
	//return SendClientMessage(id, color, _str);
}
//2.5.2
...
{
	new title_str[128];
	format(title_str, 128, "Lisek-RP ª {00b33c}%s", text);
	return title_str;
}
...
public OPCLogin(playerid)
{
	...
	if(result == -1 || result == 1) //logowanie
	{
        //Logowanie
		new string[256];
		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze Lisek Role Play! Zaloguj siÍ aby rozpoczπÊ grÍ.");
	...
	else //rejestracja
	{
        ...
        {
    		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze Lisek Role Play! Zarejestruj swoje konto aby rozpoczπÊ grÍ.");
    ...
    if(GetPlayerVirtualWorld(playerid) != 7777)
    {
    	switch(KodyAC[code])
    	{
    		...
    		case 6: // ban
    		{
    			 ...
    			 SendClientMessage(playerid, 0x9ACD32AA, string);
    			 SendClientMessage(playerid, COLOR_NEWS, "Jeúli uwaøasz ze ban jest nies≥uszny wejdü na www.Lisek-RP.pl i z≥Ûø prosbÍ o UN-BAN");
