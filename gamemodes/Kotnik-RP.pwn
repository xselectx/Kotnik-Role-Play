...
|_|\_\___/ \__|_| |_|_|_|\_\      |_|  \_\___/|_|\___|      |_|    |_|\__,_|\__, |
    ...
    | |   (_)        | |         | ___ \    | |           | ___ \ |            
    | |    _ ___  ___| | ________| |_/ /___ | | ___ ______| |_/ / | __ _ _   _ 
    | |   | / __|/ _ \ |/ /______|    // _ \| |/ _ \______|  __/| |/ _` | | | |
    | |___| \__ \  __/   <       | |\ \ (_) | |  __/      | |   | | (_| | |_| |
    \_____/_|___/\___|_|\_\      \_| \_\___/|_|\___|      \_|   |_|\__,_|\__, |
                                                                          __/ |
                                                                         |___/ 
                    
    Autor mapy: Mrucznik (github.com/mrucznik)
        |---> KotnikÆ Role Play |---> od 04.12.2020r do 02.02.2021r
        |---> LisekÆ Role Play  |---> od 02.02.2021r

    Czas dzia≥ania projektu: 04.12.2020r
    Osoby biorπce udzia≥ w rozwoju skryptu: 
        |---> xSeLeCTx (github.com/xselectx) ---> od 10.10.2020r
        |---> LukeSQLY (github.com/LukeSQLY) ---> od 04.12.2020r do 02.02.2021r
*/

// ----- <[ Wy≥πczenie fake warningÛw ]> ----- //
...
{    
    ...
    if(hittype == BULLET_HIT_TYPE_PLAYER)
    {
        ...
        new damagedid = hitid;
        GetPlayerHealth(damagedid, Float:health);
        GetPlayerArmour(damagedid, Float:ARMOR);
        if(CheckAdminGodMode(playerid, damagedid) == 0)
        {
            if(ARMOR > 0)
            {
                if(ARMOR > s_WeaponDamage[weaponid])
                {
                    //SetPlayerArmour(playerid, 0);
                    SetPlayerArmour(damagedid, ARMOR-s_WeaponDamage[weaponid]);
                } else {
                    new Float:amount2 = s_WeaponDamage[weaponid]-ARMOR;
                    SetPlayerArmour(damagedid, 0);
                    SetPlayerHealth(damagedid, health-amount2);
                    if(health-amount2 <= 0 && GetPVarInt(damagedid, "IsDead") == 0) OnPlayerDeath(damagedid, playerid, weaponid);
                }
            } else {
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    {
        if(strlen(ServerInfo) > 1) ShowPlayerDialogEx(playerid, D_SERVERINFO, DIALOG_STYLE_MSGBOX, "Lisek-RP ª Informacja", ServerInfo, "Schowaj", "Zamknij");
    }
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    ...
    if(PlayerInfo[playerid][pBW] > 0)
    {
        ...
        ShowPlayerDialogEx(playerid, D_BW, DIALOG_STYLE_MSGBOX, "Lisek Role Play", string, "Okej", "");
    }
...
public OnPlayerConnect(playerid)
{
    ...
    PlayAudioStreamForPlayer(playerid, "http://146.59.17.224/download/samp/lisek-rp.mp3",  0.0, 0.0, 0.0, 50.0, 0);
    ClearChat(playerid);	
...
public OnPlayerSpawn(playerid) //Przebudowany
{
  ...
  /*  if(GetPVarInt(playerid, "SpawnMusic") != -1)
    ...
    {
        sendTipDialogMessage(playerid, sprintf("Posiadasz aktywnπ blokadÍ posiadania broni. Pozosta≥o: %dh", PlayerInfo[playerid][pBBron]));
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(PlayerInfo[playerid][pLinia55]==1)
	{
		if(CP[playerid]==551) //Sprawdza czy dojecha≥ do nastepnego
		{
			...
			Przystanek(playerid, COLOR_BLUE, "Linia nr 55 \n Kierunek: Biurowiec 'Lisek Tower'\n{778899}Trasa: Jefferson - Glen Park - Idlewood - El Corona - Urzπd Miasta - Downtown - Market - Rodeo\n{808080}NastÍpny przystanek: Motel Jefferson");
			ProxDetector(10.0, playerid, "Linia numer: {3333FF}55 (niebieska) {FFFF00} Kierunek ==> {3333FF}Lisek TOWER", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
		...
		else if(CP[playerid]==562)
		{
			...
			_MruGracz(playerid, "|_____________>>LSBD GPS<<_____________|");
			Przystanek(playerid, COLOR_BLUE, "Linia numer 55 \nKierunek: Biurowiec 'Lisek Tower'\n{778899}Trasa: Rodeo\n{808080}NastÍpny przystanek: Lisek TOWER ");
		}
		else if(CP[playerid]==1001)
		{
			ProxDetector(10.0, playerid, "{FFFFFF}Linia 55: {FFFF00}Przystanek koÒcowy {EE82EE}Rodeo ñ Biurowiec 'Lisek Tower'", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
			_MruGracz(playerid, "+230$");
			DisablePlayerCheckpoint(playerid);
			SetPlayerCheckpoint(playerid, 341.5396,-1531.1141,32.8992, 4); //CP Lisek tower
			TogglePlayerControllable(playerid, 1);
			CP[playerid]=563;
			_MruGracz(playerid, "|_____________Komunikat_____________|");
	...
	if(PlayerInfo[playerid][pLinia72]==1)
	{
		if(CP[playerid]==721) //Sprawdza czy dojecha≥ do nastepnego
		{
			...
			_MruGracz(playerid, "Jeøeli bÍdziesz chcia≥ przerwaÊ trasÍ wczeúniej wpisz /zakoncztrase");
			_MruGracz(playerid, "|_____________>>/fare<<_____________|");
			ProxDetector(10.0, playerid, "Linia numer: {FFA500}72 (pomaraÒczowa)  {FFFF00}Kierunek ==> {FFA500}Lisek TOWER", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
		...
		{
			DisablePlayerCheckpoint(playerid);
			SetPlayerCheckpoint(playerid, 341.5396,-1531.1141,32.8992, 4); //Lisek tower
			CP[playerid]=729;
		...
		{
			DisablePlayerCheckpoint(playerid);
			SetPlayerCheckpoint(playerid, 260.5440,-1531.3761,32.1662, 4); //nawrotka przy Lisek tower
			CP[playerid]=1003;
			ProxDetector(10.0, playerid, "Przystanek koÒcowy: {EE82EE}Rodeo ñ Biurowiec 'Lisek Tower'", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			_MruGracz(playerid, "|_____________Komunikat_____________|");
			_MruGracz(playerid, "Gratulacje! UkoÒczy≥eú trasÍ do Lisek Tower! (1/2 zadania). Moøesz tu odpoczπÊ.");
    ...
    if(PlayerInfo[playerid][pLinia96]==1)
	{
		if(CP[playerid]==961) //Sprawdza czy dojecha≥ do nastepnego
		{
			...
			Przystanek(playerid, 0xAFAFAFFF, "{ADFF2F}Linia nr. 96\nKierunek: Lisek Tower\n{778899}Trasa: Ocean Docks - Lotnisko - El Corona - Commerce - Market - Marina - Rodeo\n{808080}NastÍpny przystanek: Baza Wojskowa");
			ProxDetector(10.0, playerid, "Linia numer: {ADFF2F}96 (zielona) {FFFF00}Kierunek ==> {ADFF2F}Lisek TOWER", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
		...
		else if(CP[playerid]==972)
		{
			...
			ProxDetector(10.0, playerid, "Przystanek koÒcowy: {EE82EE}Rodeo - Lisek Tower", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			_MruGracz(playerid, "|_____________Komunikat_____________|");
			_MruGracz(playerid, "Gratulacje! UkoÒczy≥eú trasÍ do Lisek Tower! (1/2 zadania). Moøesz odpoczπÊ.");
			_MruGracz(playerid, "Jeøeli chcesz jechaÊ dalej wykonaj nawrÛt kierujπc siÍ do kolejnego checkpointu!");
			_MruGracz(playerid, "|_____________>>LSBD GPS<<_____________|");
			PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
			_MruGracz(playerid, "+310$");
	...
	if(PlayerInfo[playerid][pLinia85]==1)//linia 85 system autobusÛw
	{
		...
		else if(CP[playerid]==509)
		{
			...
			CP[playerid]=510;
			Przystanek(playerid, 0xAFAFAFFF, "{FF00FF}Linia nr. 85\nKierunek: Szpital\n{778899}Trasa: Downtown - Market\n{808080}NastÍpny przystanek: Lisekowy Gun Shop");
		...
		else if(CP[playerid]==510)
		{
			...
			CP[playerid]=511;
			Przystanek(playerid, 0xAFAFAFFF, "{FF00FF}Linia nr. 85\nKierunek: Szpital\n{778899}Trasa: Downtown - Market\n{808080}NastÍpny przystanek: Verte Bank");
			ProxDetector(10.0, playerid, "{FFFFFF}Linia 85: {FFFF00}Przystanek {EE82EE}Downtown ñ Liskowy Gun Shop", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
			_MruGracz(playerid, "+270$");
		}
		...
		else if(CP[playerid]==513)
		{
			...
			CP[playerid]=514;
			Przystanek(playerid, 0xAFAFAFFF, "{FF00FF}Linia nr. 85\nKierunek: Wysypisko\n{778899}Trasa: Downtown - Glen Park - Jefferson - Las Colinas - East LS - Willowfield\n{808080}NastÍpny przystanek: Liskowy Gun Shop");
			ProxDetector(10.0, playerid, "{FFFFFF}Linia 85: {FFFF00}Przystanek {EE82EE}Downtown ñ Bank", COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
			PlayerPlaySound(playerid, 6401, 0.0, 0.0, 0.0);
			_MruGracz(playerid, "+270$");
		}
   ...
   /* if(newstate == PLAYER_STATE_DRIVER) {
    ...
    if(oldstate == PLAYER_STATE_DRIVER)
    {
        if(CurrentTunePanel[playerid][0] > 0)
        {
            ...
            CurrentTunePanel[playerid][0] = 0;
            sendTipDialogMessage(playerid, "Wyszed≥eú z panelu tuningowania!");
        }
    }
...
public OnGameModeInit()
{
    ...
    LoadMechs(); //NPC naprawiajπce auta
	...
	format(motd, sizeof(motd), "Witaj na serwerze Lisek Role Play.");
    ...
    for(new v = 0; v < CAR_End+1; v++)
	{
		...
		VehicleUID[v][vUID] = 0;
		//SetVehicleNumberPlate(v, "{1F9F06}L-RP");
    ...
    */
    ...
    print("=====================================================================================");
    printf("\tSerwer pomyúlnie uruchomiony || Lisek-Role-Play || Wersja: %s", VERSION);
    print("=====================================================================================");
	#if DEBUG == 1
...
OnPlayerLogin(playerid, password[])
{
	...
	{//poprawne has≥o
		...
		if(PlayerInfo[playerid][pBlock] == 1)
		{
			...
			_MruGracz(playerid, "[SERVER] Jeúli uwaøasz, øe konto zosta≥o zablokowane nies≥usznie napisz apelacje na: {33CCFF}www.Lisek-RP.pl");
			KickEx(playerid);
			return 1;
		}
		...
		//Powitanie:
        ClearChat(playerid);    
		format(string, sizeof(string), "Witaj, %s [UID %d | PID: %d] - Øyczymy mi≥ej gry!",nick, PlayerInfo[playerid][pUID], playerid);
        ...
        if (PlayerInfo[playerid][pCzystka] > 0)
        {
            ...
            sendTipDialogMessage(playerid, "Twoje konto zosta≥o wyczyszczone z gotÛwki, pojazdÛw, domÛw, slotÛw\n Otrzymujesz 5000$ na start.");
            KickEx(playerid);
        }
	...
	/*if(dini_Exists("Filtry.ini"))
	{
		...
		if(filtr != 0)
		{
			...
			_MruGracz(playerid,"SERVER: Aby go zdjπÊ napisz na forum www.Lisek-RP.pl");
			KickEx(playerid);
			return 1;
		}
	}
...
public OnPlayerKeyStateChange(playerid,newkeys,oldkeys)
{
            ...
            if(newkeys & KEY_NO)
            {
                if(kaska[playerid] >= FELGA_CENA)
                {
                    if(GetDistanceBetweenPlayers(playerid,GetPVarInt(playerid, "TuneWho")) < 10)
                    {
                        ...
                        format(string, sizeof(string), "Czy chcesz zakupiÊ felgi {9ACD32}%s{A9C4E4} za {9ACD32}$%d{A9C4E4}?", Felgi[CurrentTunePanel[playerid][1]], FELGA_CENA);
                        ShowPlayerDialogEx(playerid, D_TUNEPANEL, DIALOG_STYLE_MSGBOX, "Panel tuningÛw", string, "Tak", "Nie");
                    } else sendTipDialogMessage(playerid, "Mechanik jest za daleko!");
                } else sendTipDialogMessage(playerid, sprintf("Felgi kosztujπ $%d, a Ty tyle nie masz.", FELGA_CENA));
            ...
            if(newkeys & KEY_NO)
            {
                if(kaska[playerid] >= MALUNEK_CENA)
                {
                    if(GetDistanceBetweenPlayers(playerid,GetPVarInt(playerid, "TuneWho")) < 10)
                    {
                        SetPVarInt(playerid, "TunePanel", CurrentTunePanel[playerid][0]);
                        SetPVarInt(playerid, "TuneVar", CurrentTunePanel[playerid][1]);
                        format(string, sizeof(string), "Czy chcesz zakupiÊ ten maluenk za {9ACD32}$%d{A9C4E4}?", MALUNEK_CENA);
                        ShowPlayerDialogEx(playerid, D_TUNEPANEL, DIALOG_STYLE_MSGBOX, "Panel tuningÛw", string, "Tak", "Nie");
                    } else sendTipDialogMessage(playerid, "Mechanik jest za daleko!");
                } else sendTipDialogMessage(playerid, sprintf("Malunki kosztujπ $%d, a Ty tyle nie masz.", MALUNEK_CENA));
    ...
    if(newkeys & KEY_NO && (GetPlayerState(playerid)==PLAYER_STATE_DRIVER))
    {
        ...
        || IsPlayerInRangeOfPoint(playerid, 4, MechPosition[2][0], MechPosition[2][1], MechPosition[2][2]))
        {
            ...
            if(kaska[playerid] < MECHS_COST_REPAIR) return sendTipDialogMessage(playerid, sprintf("Nie staÊ CiÍ na naprawÍ pojazdu u mechanika ($%d)", MECHS_COST_REPAIR)); 
            if(GetPVarInt(playerid, "botnaprawia") == 1) return sendTipDialogMessage(playerid, "Naprawiasz juø ten pojazd!");
            if(engine == 1) return sendTipDialogMessage(playerid, "Zgaú silnik!");  
            if(carhealth < 500.0)
            {
                ...
                //else return sendTipDialogMessage(playerid, "TwÛj pojazd nie moøe zostaÊ naprawiony!");
            } else sendTipDialogMessage(playerid, "TwÛj pojazd nie moøe zostaÊ naprawiony!");
        }
    }
    //
...
public OnPlayerText(playerid, text[])
{
	...
	if (realchat)
	{
        ...
        format(string, sizeof(string), "%s mÛwi: %s", GetNick(playerid, true), newtext);
        ProxDetectorEx(10.0, playerid, string, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
        ...
        else
        {
            ...
            if(pos != -1)
            {
                ...
                ProxDetectorEx(13.0, playerid, string, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
