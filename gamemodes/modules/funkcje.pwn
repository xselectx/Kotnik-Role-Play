...
sendTipDialogMessage(id, string:msg[]) {
	...
	return ShowPlayerDialogEx(id, DIALOG_ID_NO_RESPONSE, DIALOG_STYLE_MSGBOX, "{8FCB04}Kotnik-RP{FFFFFF} ª Informacja", _str, "Zamknij", "");
	//return SendClientMessage(id, color, _str);
}

sendErrorDialogMessage(id, string:msg[]) {
	format(_str,128,"%s", msg);
	return ShowPlayerDialogEx(id, DIALOG_ID_NO_RESPONSE, DIALOG_STYLE_MSGBOX, "{8FCB04}Kotnik-RP{FFFFFF} ª Wystπpi≥ b≥πd", _str, "Zamknij", "");
	//return SendClientMessage(id, color, _str);
}
//2.5.2
...
stock Czystka(playerid) {
	...
	sendTipMessage(playerid, "Twoje konto zosta≥o wyczyszczone, usuniÍto Ci dom, gotÛwkÍ, pojazdy, sloty. Otrzymujesz 5000$ na start");
}
...
{
	new title_str[128];
	format(title_str, 128, "Kotnik-RP ª {00b33c}%s", text);
	return title_str;
}
...
public zabezpieczeniewoz(playerid)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
	    SendClientMessage(playerid, COLOR_PANICRED, "Dosta≥eú kicka za prawdopodobieÒstwo uøycia czitÛw. Jeúli to bug, napisz na forum: www.Kotnik-rp.net");
		KickEx(playerid);
	}
	return 1;
}*/
...
//alarm siÍ w≥πcza
    if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
    	...
    	format(pdinfo, sizeof(pdinfo), "[911]: Zg≥oszono prÛbÍ kradzieøy pojazdu %s w okolicy %s ",VehicleNames[GetVehicleModel(pojazd)-400], pZone);
...
CanUseCar(playerid, newcar)
{
	...
	{
        sendErrorDialogMessage(playerid, sprintf("Posiadasz aktywnπ blokadÍ prowadzenia pojazdÛw. Pozosta≥o: %dh", PlayerInfo[playerid][pBPojazd]));
        return 0;
    }
	...
	if(VehicleUID[newcar][vUID] != 0)
    {
	    ...
	    if(CarData[lcarid][c_OwnerType] == CAR_OWNER_FRACTION)// wszystkie auta frakcji
	    {
            ...
            if(CarData[lcarid][c_Owner] == 11)
            {
                ...
                if(GetPlayerFraction(playerid) == 11 && PlayerInfo[playerid][pRank] < CarData[lcarid][c_Rang])
	        	{
		        	...
		        	//sendTipMessageEx(playerid,COLOR_GREY,string);
		        	sendTipMessageEx(playerid, COLOR_GREY, sprintf("Nie moøesz uøywaÊ tego pojazdu! Wymagana ranga: [%d] %s!", CarData[lID][c_Rang], FracRang[own][rank]));
		        	return 0;
	        	}
        ...
        else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_SPECIAL) //specjalne
        {
            /*if(CarData[lcarid][c_Owner] == RENT_CAR)
            {
    			...
    			{
    			    sendTipMessageEx(playerid, COLOR_LIGHTBLUE, "Ten pojazd naleøy do wypoøyczalni! Nie wypoøyczy≥eú go!");
        ...
        else if(CarData[lcarid][c_OwnerType] == CAR_OWNER_FAMILY)// wszystkie auta RODZIN
	    {
	        ...
	        if(PlayerInfo[playerid][pRank] < CarData[lcarid][c_Rang])
	        {
				...
				sendTipMessageEx(playerid, COLOR_GREY, sprintf("Nie moøesz uøywaÊ tego pojazdu! Wymagana ranga: [%d] %s!", CarData[lID][c_Rang], FamRang[own][rank]));
                //format(string, sizeof(string), "Aby kierowaÊ tym pojazdem potrzebujesz %d rangi!", CarData[lcarid][c_Rang]);
		        //sendTipMessageEx(playerid,COLOR_GREY,string);
		        return 0;
	        }
...
/*CanUseCar(pid, carid)
{
	...
	if(CarData[lID][c_OwnerType] == CAR_OWNER_PLAYER)
	{
		...
		{
			 //_MruGracz(pid, "Moøesz uøywaÊ tego auta (PRYWATNE AUTO)");
			return 1;
		} else 
		{
			SendClientMessage(pid, COLOR_NEWS, "Nie posiadasz kluczykÛw do tego pojazdu, a zabezpieczenia sπ zbyt dobre byú mÛg≥ go ukraúÊ!");
			return 0;
		}
	}
	else if(CarData[lID][c_OwnerType] == CAR_OWNER_FRACTION) // rodziny/org
	{
		if(CarData[lID][c_Owner] == PlayerInfo[pid][pMember] || PlayerInfo[pid][pLider] == CarData[lID][c_Owner]) // gracz ma frakcje taka sama jakie auto
		{
			...
			{
				//_MruGracz(pid, "Moøesz uøywaÊ tego pojazdu!");
				return 1;
			} else {
				new own = CarData[lID][c_Owner];
				new rank = CarData[lID][c_Rang];
				sendTipMessage(pid, sprintf("Nie moøesz uøywaÊ tego pojazdu! Wymagana ranga: [%d] %s!", CarData[lID][c_Rang], FracRang[own][rank]));
				return 0;
			}
		}
		else { // nie moze bo nie jest we frakcji
			sendTipMessage(pid, sprintf("Nie moøesz uøywaÊ tego pojazdu, ten pojazd naleøy do %s!", FractionNames[CarData[lID][c_Owner]]));
			return 0;
		}
	}
	else if(CarData[lID][c_OwnerType] == CAR_OWNER_FAMILY) // rodziny/org
	{
		if(CarData[lID][c_Owner] == PlayerInfo[pid][pOrg]) // gracz ma rodzine taka sama jakie auto
		{
			if(CarData[lID][c_Rang] <= PlayerInfo[pid][pRank] ) // ma odpowiednia range
			{
				...
				new own = CarData[lID][c_Owner];
				new rank = CarData[lID][c_Rang];
				sendTipMessage(pid, sprintf("Nie moøesz uøywaÊ tego pojazdu! Wymagana ranga: [%d] %s!", CarData[lID][c_Rang], FamRang[own][rank]));
				return 0;
			}
		}
		else { // nie moze bo nie jest w rodzinie
			sendTipMessage(pid, "Nie moøesz uøywaÊ tego pojazdu, ten pojazd naleøy do rodziny!");
			return 0;
		}
	}
	else if(CarData[lID][c_OwnerType] == CAR_OWNER_JOB) // prace dorywcze
	{
		if(CarData[lID][c_Owner] == PlayerInfo[pid][pJob])
		{
			...
			switch(CarData[lID][c_Owner])
			{
				...
				case 11: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [11]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 12: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [12]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 13: // bokser
				{
				...
				case 14: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [14]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 15: // gazeciarz
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [15] (Gazeciarz). Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 16: // bokser
				{
				...
				case 18: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [18]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 19: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [19]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				case 20: // brak
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [20]. Zg≥oú ten fakt administracji!");
					return 0;
				}
				default: 
				{
					sendErrorMessage(pid, "Ten pojazd ma niepoprawnπ pracÍ [default]. Zg≥oú ten fakt administracji!");
					return 0;
				}
			}
			if(error_drive == 1)
			{
				sendTipMessage(pid, sprintf("Nie moøesz uøywaÊ tego pojazdu! Wymagany skill %d!", CarData[lID][c_Rang]));
				return 0;
			}
		} else {
			sendTipMessage(pid, sprintf("Ten pojazd naleøy do zawodu %s!", JobNames[CarData[lID][c_Owner]]));
			return 0;
		}
	}
	return 1;
...
ShowStats(playerid,targetid)
{
    if(IsPlayerConnected(playerid) && IsPlayerConnected(targetid))
	{
		...
		format(coordsstring, sizeof(coordsstring), "Level:[%d] P≥eÊ:[%s] Wiek:[%d] Pochodzenie:[%s] Zdrowie:[%.1f] Kasa:[$%d] Bank:[$%d] Telefon:[%d]", level,atext,age,otext,50+shealth, cash, account, pnumber);
...
public ProxDetectorEx(Float:radi, playerid, string[],col1,col2,col3,col4,col5)
{
    if(IsPlayerConnected(playerid))
    {
        ...
        if(strfind(string, "mÛwi: ", true) == -1)
        {
        	strreplace(string, "mÛwi:", "mÛwi: ", true, 0, -1, 256);
        }
...
public OPCLogin(playerid)
{
	...
	if(result == -1 || result == 1) //logowanie
	{
        //Logowanie
		new string[256];
		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze Kotnik Role Play! Zaloguj siÍ aby rozpoczπÊ grÍ.");
		//SendClientMessage(playerid, COLOR_LIGHTRED, "Witaj na serwerze Kotnik Role Play BETA! Wszystkie konta zostanπ wyzerowane i skasowane na start serwera!");
	...
	else //rejestracja
	{
        ...
        {
    		SendClientMessage(playerid, COLOR_YELLOW, "Witaj na serwerze Kotnik Role Play! Zarejestruj swoje konto aby rozpoczπÊ grÍ.");
...
stock LoadConfig()
{
    ...
    print("[K:RP]: Wczytano podstawowπ konfiguracjÍ");
    mysql_free_result();
}
...
stock CheckWrongCharacters(string[])
{
    ...
    else if(strfind(string,"π",true) != (-1)) return 0;
    else if(strfind(string,"Ê",true) != (-1)) return 0;
    else if(strfind(string,"Í",true) != (-1)) return 0;
    else if(strfind(string,"≥",true) != (-1)) return 0;
    else if(strfind(string,"Ò",true) != (-1)) return 0;
    else if(strfind(string,"Û",true) != (-1)) return 0;
    else if(strfind(string,"ú",true) != (-1)) return 0;
    else if(strfind(string,"ø",true) != (-1)) return 0;
    else if(strfind(string,"ü",true) != (-1)) return 0;
    else if(strfind(string,"!",true) != (-1)) return 0;
    else if(strfind(string,"%",true) != (-1)) return 0;
    else if(strfind(string,"#",true) != (-1)) return 0;
    else if(strfind(string,"^",true) != (-1)) return 0;
...
stock Oil_Destroy(lID)
{
    ...
    foreach(Player, i)
    {
        ...
        SendClientMessage(i, COLOR_WHITE, "[LSFD] Usunπ≥eú plamÍ oleju! Otrzymujesz 15 000$! [LSFD]");
        DajKase(i, 15000);
        SendFamilyMessage(4, COLOR_GREEN, "[LSFD] Straøak usunπ≥ plamÍ oleju! Na konto frakcji wp≥ywa 15 000$! [LSFD]");
        Sejf_Add(4, 15000);
    }
}
...
Wybieralka_Setup(playerid)
{
	...
	SendClientMessage(playerid, COLOR_BLUE, "Wybierz interesujπcy ciÍ skin");
    ShowPlayerDialogEx(playerid, D_UBRANIA, DIALOG_STYLE_LIST, "Wybierz kategoriÍ skina", "Skiny mÍskie\nSkiny damskie", "Wybierz", "Anuluj");
...
public OnCheatDetected(playerid, ip_address[], type, code)
{
    ...
    if(GetPlayerVirtualWorld(playerid) != 7777)
    {
    	switch(KodyAC[code])
    	{
    		...
    		case 3: // Ostrzeøenie (/cziterzy)
    		{
    			SetPVarInt(playerid, "AC-warn", GetPVarInt(playerid, "AC-Warn")+1);
    			return 3;

    		}
    		case 4: // AdmWarning + Ostrzeøenie (/cziterzy)
    		{
    			format(string, sizeof(string), "AC: %s [%d] najprawdopodobniej czituje! | %s.", GetNick(playerid), playerid, code_decoded);
    			SendAdminMessage(COLOR3, string);
    			SetPVarInt(playerid, "AC-warn", GetPVarInt(playerid, "AC-Warn")+1);
    		...
    		case 5: // AdmWarning + Ostrzeøenie (/cziterzy) raz na sekundÍ
    		{
    		...
    		case 6: // ban
    		{
    			 ...
    			 SendAdminMessage(0x9ACD32AA, string);
    			 BanLog(string);
    			 format(string, sizeof(string), "Anti-Cheat: Zosta≥eú zbanowany. | Kod: %d.", code);
    			 SendClientMessage(playerid, 0x9ACD32AA, string);
    			 SendClientMessage(playerid, COLOR_NEWS, "Jeúli uwaøasz ze ban jest nies≥uszny wejdü na www.Kotnik-RP.pl i z≥Ûø prosbÍ o UN-BAN");
...
ShowKody(playerid, page = 1)
{
    ...
    if(page > 1) strcat(string, "´´\t\tPoprzednia strona");
   	...
   	ShowPlayerDialogEx(playerid, D_ANTYCHEAT, DIALOG_STYLE_TABLIST_HEADERS, sprintf("Antycheat (%d/%d)", page, pages), string, "ZmieÒ", "Wyjdü");
    ...
    /*if(page == 1)
    {
    	...
    	ShowPlayerDialogEx(playerid, D_ANTYCHEAT, DIALOG_STYLE_TABLIST_HEADERS, "Antycheat (1/2)", string, "ZmieÒ", "Wyjdü");
	}
	else if(page == 2)
	{
    	...
    	//printf("%s", string);
    	ShowPlayerDialogEx(playerid, D_ANTYCHEAT+1, DIALOG_STYLE_TABLIST_HEADERS, "Antycheat (2/2)", string, "ZmieÒ", "Wyjdü");
	}*/
	return 1;
}
...
stock ShowPrzystanki(playerid, page)
{
    ...
    if(page == 1)
    {
        ...
        format(lStr, sizeof(lStr), "%s" "\t" "\tNastÍpna strona", lStr);
        ShowPlayerDialogEx(playerid, D_PRZYSTANEK, DIALOG_STYLE_TABLIST_HEADERS, "Przystanki autobusowe (1/3)", lStr, "Jedü", "Anuluj");
    } 
    else if (page == 2) 
    {
        ...
        format(lStr, sizeof(lStr), "%sPoprzednia Strona\n" "\t" "\tNastÍpna strona", lStr);
        ShowPlayerDialogEx(playerid, D_PRZYSTANEK+1, DIALOG_STYLE_TABLIST_HEADERS,"Przystanki autobusowe (2/3)", lStr, "Jedü", "Anuluj");
    } 
    else if (page == 3)
    {
        ...
        ShowPlayerDialogEx(playerid, D_PRZYSTANEK+2, DIALOG_STYLE_TABLIST_HEADERS,"Przystanki autobusowe (3/3)", lStr, "Jedü", "Anuluj");
    }
    return 1;
}
...
PrzystanekRide(playerid, listitem)
{
    ...
    if(kasa > kaska[playerid]) return sendTipMessage(playerid, "Nie staÊ CiÍ!");
    ...
    format(string, sizeof(string), "* %s wsiada do autobusu, ktÛry jedzie w kierunku %s.", GetNick(playerid, true), PrzystankiAutobusowe[listitem][PrzystanekName]);
	ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
	return 1;
}
    ...
    if(GetPlayerState(playerid)==PLAYER_STATE_DRIVER || GetPlayerState(playerid) == PLAYER_STATE_ENTER_VEHICLE_DRIVER)
    {
        ...
        if(oldCarHP[playerid] - health2 > 60 && oldCarHP[playerid] - health < 200)
        {
            if(!IsARower(vehicleid) && !IsABike(vehicleid))
            {
            	if(pasy[playerid] == 0)
            	{
            	    format(string, sizeof(string), "* %s uderzy≥ g≥owπ w kierownicÍ.", GetNick(playerid, true));
            	    ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            	...
            	if(pasy[playerid] == 1)
            	{
            	    format(string, sizeof(string), "* Pasy zadzia≥a≥y i %s nie dozna≥ powaønych obraøeÒ.", GetNick(playerid, true));
            	    ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            	}
        	}
        	else 
        	{
            	...
            	if(kask[playerid] == 1)
            	{
            	    format(string, sizeof(string), "* Kask uratowa≥ %s i nie dozna≥ on powaønych obraøeÒ.", GetNick(playerid, true));
            	    ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            	}
        	}
        }
        if(oldCarHP[playerid] - health2 > 200)
        {
            if(!IsARower(vehicleid) && !IsABike(vehicleid))
            {
            	...
            	if(pasy[playerid] == 1)
            	{
            	    format(string, sizeof(string), "* %s uderzy≥ g≥owπ w kierownicÍ pomimo zapiÍtych pasÛw.", GetNick(playerid, true));
            	    ProxDetector(10.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
            	    new math = floatround(((oldCarHP[playerid] - health2) / 10) * 0.3);
            	    SetPlayerHealth(playerid, playerhealth2 - math);
            	}
...
stock hq_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 1596)
    {
        ...
        if(PlayerInfo[playerid][pMember] == 1 || PlayerInfo[playerid][pLider] == 1)
        {
            ...
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"TreúÊ: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_tresc]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Godzina: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_kiedy]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Zg≥osi≥: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_nadal]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Obszar: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_lokacja]);
            new statusTxt[30];
            switch(Zgloszenie[dg_data][zgloszenie_status]) {
                case 0: statusTxt = "Nie podjÍto jeszcze decyzji";
            ...
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Status: "#HQ_COLOR_TEKST2"%s", string, statusTxt);
            format(string, sizeof(string), "%s\n ", string);
            if(Zgloszenie[dg_data][zgloszenie_status] == 1) {
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Przyjπ≥: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
                ...
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Odrzuci≥: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
            } else {
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Wyda≥ decyzjÍ: "#HQ_COLOR_TEKST2"%s", string, Zgloszenie[dg_data][zgloszenie_przyjal]);
            }

            SetPVarInt(playerid, "zarzadzajZgl", dg_data);

            ShowPlayerDialogEx(playerid, 1597, DIALOG_STYLE_LIST, "SzczegÛ≥y Zgloszenia", string, "Wybierz", "WrÛÊ");
        }
        if(PlayerInfo[playerid][pMember] == 3 || PlayerInfo[playerid][pLider] == 3)
        {
            ...
            format(string, sizeof(string), ""#HQ_COLOR_TEKST2"Informacja o zg≥oszeniu");
            format(string, sizeof(string), "%s\n"#HQ_COLOR_PLACEHOLDER"====================================================", string);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"TreúÊ: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_tresc]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Godzina: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_kiedy]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Zg≥osi≥: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_nadal]);
            format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Obszar: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_lokacja]);
            new statusTxt[30];
            ...
            if(ZgloszenieSasp[dg_data][zgloszenie_status] == 1) {
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Przyjπ≥: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
                ...
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Odrzuci≥: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
            } else {
                format(string, sizeof(string), "%s\n"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"Wyda≥ decyzjÍ: "#HQ_COLOR_TEKST2"%s", string, ZgloszenieSasp[dg_data][zgloszenie_przyjal]);
            }
    ...
    if(dialogid == 1597)
    {
        ...
        if(PlayerInfo[playerid][pMember] == 1 || PlayerInfo[playerid][pLider] == 1)
        {
            ...
            if(listitem == 9)
            {
                ...
                if(zglstatus == 0)
                {
                    if(!strcmp(Zgloszenie[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zg≥oszeÒ nie moøna akceptowaÊ!");
        ...
        if(PlayerInfo[playerid][pMember] == 3 || PlayerInfo[playerid][pLider] == 3)
        {
            ...
            if(listitem == 9)
            {
                ...
                if(zglstatus == 0)
                {
                    if(!strcmp(ZgloszenieSasp[szczegol][zgloszenie_nadal], "Brak")) return sendErrorMessage(playerid, "Pustych zg≥oszeÒ nie moøna akceptowaÊ!");
...
stock ShowHeadquarters(playerid, page=HQ_MAIN)
{
    if(page == HQ_MAIN)
    {
        ...
        ShowPlayerDialogEx(playerid, 1595, DIALOG_STYLE_LIST, "HeadQuarters/MDC", string, "Ok", "Wyjdü");
    }
    if(page == HQ_WL)
    {
        cmd_wanted(playerid);
        ...
        if(PlayerInfo[playerid][pMember] == 1 || PlayerInfo[playerid][pLider] == 1)
        {
            for(new i = 0, j=OSTATNIE_ZGLOSZENIA; i<j; i++) {
                if(Zgloszenie[i][zgloszenie_status] == 0) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_TEKST"%s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                } else if(Zgloszenie[i][zgloszenie_status] == 1) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_AKCEPTOWANE"%s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                } else if(Zgloszenie[i][zgloszenie_status] == 2) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_ODRZUCONE"[Odrzucone] %s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                } else if(Zgloszenie[i][zgloszenie_status] == 3) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_FALSZYWE"[Fa≥szywe] %s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                } else if(Zgloszenie[i][zgloszenie_status] == 4) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_WYKONANE"[Wykonane] %s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                } else if(Zgloszenie[i][zgloszenie_status] == 5) {
                    format(string, sizeof(string), "%s"#HQ_COLOR_STRZALKA"    ªª "#HQ_COLOR_ANULOWANE"[Anulowane] %s\n", string, Zgloszenie[i][zgloszenie_tresc]);
                }
...
stock fPanel_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == 1960)
	{
    	...
    	if(listitem == 3)
    	{
			...
			if(ReturnExactUser(pracownik_nick) != INVALID_PLAYER_ID) {
                //sendTipMessage(playerid, "Gracz jest online, uøyj /zwolnij");
                found = 1;
                new str[4];
            ...
            if(found == 0)
            {
            	format(msg, sizeof(msg), "Zwolni≥eú %s ze swojej frakcji", pracownik_nick, FractionNames[PlayerInfo[playerid][pLider]]);
    	...
    	else if(listitem == 4)
    	{
		    ...
		    return ShowPlayerDialogEx(playerid, 1966, DIALOG_STYLE_LIST, "Wybierz rangÍ, ktÛrπ chcesz nadaÊ graczu", str, "Nadaj", "Anuluj");
    	}
    	else
    	{
	...
	if(dialogid == 1966)
	{
		...
		if(ReturnExactUser(pracownik_nick) != INVALID_PLAYER_ID) {
            ...
            //sendTipMessage(playerid, "Gracz jest online, uøyj /dajrange");
            //return showFactionWorkers(playerid, GetPVarInt(playerid, "fpanel_Page"));
        }
		...
		if(found == 0)
		{
			format(msg, sizeof(msg), "Awansowa≥eú %s na rangÍ %s", pracownik_nick, FracRang[PlayerInfo[playerid][pLider]][listitem]);
        	sendTipMessage(playerid, msg, COLOR_LIGHTBLUE);
    	}
...
stock showEmployeeInfo(playerid, employeeUid)
{
    ...
    if(isLider > 0) {
        sendTipMessage(playerid, "DostÍp do danych na temat osÛb z panelem zabroniony", COLOR_LIGHTBLUE);
        return showFactionWorkers(playerid, GetPVarInt(playerid, "fpanel_Page"));
    }
    strmid(rankname, FracRang[dorangi][ranga], 0, 25, 26);
    format(employeestring, sizeof(employeestring), ""#KARA_STRZALKA"    ªª "#KARA_TEKST"Nick: "#KARA_TEKST"%s", pracownik_nick);
    format(employeestring, sizeof(employeestring), "%s\n"#KARA_STRZALKA"    ªª "#KARA_TEKST"Ranga: "#KARA_TEKST"%s", employeestring, rankname);
    format(employeestring, sizeof(employeestring), "%s\n ", employeestring);
    format(employeestring, sizeof(employeestring), "%s\n"#HQ_COLOR_STRZALKA"    ªª {dafc10}WyrzuÊ Pracownika", employeestring);  
    format(employeestring, sizeof(employeestring), "%s\n"#HQ_COLOR_STRZALKA"    ªª {dafc10}ZmieÒ rangÍ", employeestring);  
    SetPVarInt(playerid, "fpanel_uid", employeeUid);
    ShowPlayerDialogEx(playerid, 1960, DIALOG_STYLE_LIST, "Panel Lidera ª Zarzπdzanie Pracownikiem", employeestring, "Ok", "Wstecz");
    return 1;
}
...
stock showFactionWorkers(playerid, page=1)
{
	...
	if(ilosc == 0) return sendErrorMessage(playerid, "Ta frakcja nie ma pracownikÛw");

	new header[128], Float:pp = ilosc / FPANEL_PER_PAGE;
	format(header, sizeof(header), "Panel Lidera ª Pracownicy [%d/%d]", page, floatround(pp, floatround_ceil)+1);
...
stock factionLeaderPanel(playerid, page = FPANEL_MAIN)
{
	if(page == FPANEL_MAIN)
	{
		...
		new ftitle[130];
		new id = PlayerInfo[playerid][pLider];
    	format(mainstring, sizeof(mainstring), ""#KARA_TEKST"ªª Zarzπdzanie pracownikami");
    	format(mainstring, sizeof(mainstring), "%s\n"#KARA_TEKST"ªª Zarzπdzanie pojazdami", mainstring);
    	format(mainstring, sizeof(mainstring), "%s\n"#KARA_TEKST"ªª Zarzπdzanie skinami", mainstring);
    	format(mainstring, sizeof(mainstring), "%s\n"#KARA_TEKST"ªª Zarzπdzanie sejfem", mainstring);
    	format(ftitle, sizeof(ftitle), "%s", FractionNames[id]);
    	ShowPlayerDialogEx(playerid, 1958, DIALOG_STYLE_LIST, ftitle, mainstring, "Ok", "Wyjdü");
	}
	...
	{
		sendTipMessage(playerid, "Ta opcja bÍdzie dostÍpna wkrÛtce");
...
ShowPlayerSkins(id, type)
{
    ...
    if(type == 0)
    {
		...
		if(ilosc <= 0 ) return sendTipMessage(id, "Brak ubraÒ! Moøesz je zakupiÊ w sklepie z ubraniami!");
	...
	else if(type == 1)
	{
		...
		if(ilosc <= 0 ) return sendTipMessage(id, "Brak ubraÒ personalnych! Moøesz je zakupiÊ pod komendπ /premium!");
		return ShowPlayerDialogEx(id, D_SHOWSKINS, DIALOG_STYLE_PREVIEW_MODEL, "Ubrania", string, "Wybierz", "Anuluj");
	}
	else if(type == 2)
	{
...
public VPNCheck(playerid, response_code, data[])
{
	...
	{	
		if(strfind(data, "yes", true) != -1)
		{
	    	...
	    	SendClientMessage(playerid, 0xFF0000FF, "Zosta≥eú wyrzucony z powodu posiadania VPN/proxy!");
	    	SendClientMessage(playerid, 0xFF0000FF, "Jeøeli uwaøasz, øe to b≥πd zg≥oú ten fakt na forum lub discord.");
	    	KickEx(playerid);
		}
...
ShowPowiazania(playerid, id, typ)
{
	...
	while(mysql_fetch_row_format(query, "|"))
    {
		...
		if(uid != PlayerInfo[playerid][pUID])
		{
			if(ilosc < 15)
			{
				...
				//{
				//	format(string, sizeof(string), "%sªª %s [%d] {FF0000}%s{FFFFFF}\n", string, nick, uid, ban_reason);
				//}
				//else
				//{
					format(string, sizeof(string), "%sªª %s [%d]\n", string, nick, uid);
				//}
			}
			ilosc++;
		}
	...
	if(ilosc > 0) 
	{
		if(typ == 1) return ShowPlayerDialogEx(playerid, D_POWIAZANIA_GPCI, DIALOG_STYLE_LIST, sprintf("%s | %d powiπzaÒ GPCI", GetNick(playerid), ilosc), string, "OK", "");
		else if(typ == 0) return ShowPlayerDialogEx(playerid, D_POWIAZANIA_GPCI, DIALOG_STYLE_LIST, sprintf("%s | %d powiπzaÒ IP", GetNick(playerid), ilosc), string, "OK", "");
	}
	else return sendTipMessage(playerid, "Brak powiπzaÒ.");
...
PrintDamageLog(playerid, typ, id)
{
	...
	if(typ == 0)
	{
		for(new i = 0; i<10; i++)
		{
			...
			{
				format(string, sizeof(string), "ª %s | %s zada≥ %s %.02f obraøeÒ z %s", 
...
public OnPlayerFakeKill(playerid, spoofedid, spoofedreason, faketype)
{
	...
	if(faketype == 2)
	{
		format(string, sizeof(string), "[FakeKill] %s[%d] uøy≥ fake killa na %s[%d], reason: %d", GetNick(playerid), playerid, GetNick(spoofedid), spoofedid, spoofedreason);
		SendCommandLogMessage(string);
		OnCheatDetected(playerid, "", 1, 56);
	}
	else
	{
		format(string, sizeof(string), "[FakeKill] %s[%d] prawdopodobnie uøy≥ fake killa na %s[%d], reason: %d", GetNick(playerid), playerid, GetNick(spoofedid), spoofedid, spoofedreason);
		SendCommandLogMessage(string);
	}
}
...
{
	strreplace(source, ":D", "**úmieje siÍ**", false, 0, -1, 256);
    strreplace(source, ":d", "**wystawia jÍzyk**", false, 0, -1, 256);
    strreplace(source, ":P", "**wystawia jÍzyk**", true, 0, -1, 256);
    strreplace(source, "XD", "**wybucha úmiechem**", true, 0, -1, 256);
    strreplace(source, "X-D", "**wybucha úmiechem**", true, 0, -1, 256);
    strreplace(source, "X_D", "**wybucha úmiechem**", true, 0, -1, 256);
    strreplace(source, ":)", "**uúmiecha siÍ**", false, 0, -1, 256);
    strreplace(source, "=D", "**uúmiecha siÍ**", false, 0, -1, 256);
    strreplace(source, "8)", "**uúmiecha siÍ**", false, 0, -1, 256);
    strreplace(source, "c:", "**uúmiecha siÍ**", false, 0, -1, 256);
    strreplace(source, ":]", "**uúmiecha siÍ**", false, 0, -1, 256);
    strreplace(source, ";)", "**puszcza oczko**", false, 0, -1, 256);
    strreplace(source, ":/", "**krzywi siÍ**", false, 0, -1, 256);
    strreplace(source, ";/", "**krzywi siÍ**", false, 0, -1, 256);
    strreplace(source, ":o", "**dziwi siÍ**", true, 0, -1, 256);
    strreplace(source, ":3", "**robi s≥odkπ minÍ**", false, 0, -1, 256);
    ...
    strreplace(source, ":(", "**smuci siÍ**", false, 0, -1, 256);
    strreplace(source, ";(", "**smuci siÍ i puszcza ≥ezkÍ**", false, 0, -1, 256);
    strreplace(source, ":c", "**smuci siÍ**", true, 0, -1, 256);
    strreplace(source, ":<", "**smuci siÍ**", true, 0, -1, 256);
    strreplace(source, ":[", "**smuci siÍ**", true, 0, -1, 256);
    strreplace(source, ">:[", "**robi wnerwionπ minÍ**", true, 0, -1, 256);
    strreplace(source, ">:)", "**robi z≥owrogπ minÍ**", false, 0, -1, 256);
    strreplace(source, ">;)", "**robi z≥owrogπ minÍ**", false, 0, -1, 256);
...
/*ShowPlayerTunePanel(playerid, panel) // system podglπdu i paneli tuningÛw do 1.1.2
{
