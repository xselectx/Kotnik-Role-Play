...
wyczyscgraffiti(playerid,oID)
{
	if(gPlayerLogged[playerid] == 1)
	{
		...
		{
			sendErrorMessage(playerid, "U¿yj: /wyczyscgraffiti [oID] (wpisz /ginfo ¿eby zobaczyæ oID)");
			return 1;
		}*/
		if(gCache[oID][gUID] != 0)
		{
			...
			} else return ShowPlayerDialogEx(playerid, D_GRAFFITI_CLEAR, DIALOG_STYLE_INPUT, "Graffiti (czyszczenie)", "Jesteœ za daleko od wybranego graffiti!\n\nWybierz oID graffiti, które chcesz wyczyœciæ\nWpisz /ginfo lub wybierz z listy INFO, ¿eby zobaczyæ oID graffiti", "OK", "Anuluj");
